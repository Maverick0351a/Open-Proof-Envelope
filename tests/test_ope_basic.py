...
from odin_ope.models import BundleModel, EnvelopeModel
from odin_ope.signers import FileSigner
from odin_ope.verify import build_jwks_for_signers, verify_envelope, verify_envelope_or_raise


def test_canonical_and_cid_deterministic() -> None:
    obj = {"b": 2, "a": 1}
...
def test_sign_and_verify_envelope() -> None:
...
def test_bundle_sign_verify() -> None:
    seed_b64u = "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"
    signer = FileSigner(seed_b64u)
    receipts = [
...
def test_canonical_json_empty() -> None:
    # Edge: empty dict/list
    assert canonical_json({}) == b"{}"
    assert canonical_json([]) == b"[]"


def test_canonical_json_non_ascii() -> None:
...
def test_build_envelope_missing_fields() -> None:
...
def test_signer_invalid_seed() -> None:
    # Edge: wrong seed length
    with pytest.raises(ValueError):
        FileSigner("AA")


def test_verify_envelope_missing_sig() -> None:
...
def test_verify_envelope_kid_not_found() -> None:
...
def test_verify_envelope_cid_mismatch() -> None:
...
def test_verify_envelope_or_raise_exceptions() -> None:
    seed_b64u = "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"
    signer = FileSigner(seed_b64u)
    env = build_envelope({"x": 1}, "t", "tgt")
...
def test_verify_envelope_timestamp_skew() -> None:
    seed_b64u = "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"
    signer = FileSigner(seed_b64u)
    # Force old timestamp
...
def test_verify_bundle_or_raise() -> None:
    seed_b64u = "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"
    signer = FileSigner(seed_b64u)
    receipts = [
        {"hop": 0, "receipt_hash": "h0", "prev_receipt_hash": None},
...
def test_vector_basic() -> None:
    ...
    _ = FileSigner(data["seed_b64u"])  # recreate signer (not used directly further)
    env = {
...
def test_bundle_hop_continuity_validation() -> None:
    seed_b64u = "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"
    signer = FileSigner(seed_b64u)
    # Broken hop order
    receipts = [
...
def test_bundle_prev_hash_chain_validation() -> None:
    seed_b64u = "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"
    signer = FileSigner(seed_b64u)
    receipts = [
        {"hop": 0, "receipt_hash": "h0", "prev_receipt_hash": None},
...
def test_envelope_not_before_and_expired() -> None:
    seed_b64u = "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"
    signer = FileSigner(seed_b64u)
...
def test_canonical_json_stability(obj: dict[str, object]) -> None:
